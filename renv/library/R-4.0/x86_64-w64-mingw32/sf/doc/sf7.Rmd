...
and is available.
After `sf` has been attached, you can check if `s2` is available
as follows:
...
Most of the package's functions start with `s2_` in the same way that most `sf`
...
```{r eval=s2_available, fig.show='hold', out.width="50%"}
