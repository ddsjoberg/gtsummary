...
static void K_bessel(Float *x, Float *alpha, int *nb,
		     int *ize, Float *bk, int *ncalc);

template<class Float>
Float bessel_k(Float x, Float alpha, double expo)
{
    int nb, ncalc, ize;
    Float *bk;
#ifndef MATHLIB_STANDALONE
    ...
    bk = (Float *) calloc(nb, sizeof(Float));
    if (!bk) MATHLIB_ERROR("%s", _("bessel_k allocation error"));
#else
    vmax = vmaxget();
    bk = (Float *) R_alloc((size_t) nb, sizeof(Float));
#endif
...
/* modified version of bessel_k that accepts a work array instead of
   allocating one. */
template<class Float>
Float bessel_k_ex(Float x, Float alpha, double expo, Float *bk)
{
    int nb, ncalc, ize;
...
*/
    ...
    *ncalc = min0(*nb,0) - 2;
	...
	k = 0;
	if (nu < sqxmin_BESS_K) {
	    // Kasper: Don't clear the derivatives of nu !
	    // nu = 0.;
	} else if (nu > .5) {
	    k = 1;
	    nu -= 1.;
	...
	*ncalc = *nb;
	...
	m = min0((int) trunc(wminf - nu),iend);
	for (i = 2; i <= m; ++i) {
	    t1 = bk1;
	    bk1 = bk2;
	    twonu += 2.;
