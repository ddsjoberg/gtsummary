...
* Added theme elements `add_ci.tbl_summary-attr:method.categorical`, `add_ci.tbl_summary-attr:method.continous`, `add_ci.tbl_svysummary-attr:method.categorical` and `add_ci.tbl_svysummary-attr:method.continuous` to control the default value for `add_ci.tbl_summary(method)`/`add_ci.tbl_svysummary(method)` argument. (#2356)
