...
tidy_wrap <- function(x, exponentiate, conf.level) {
  ...
  if (!(class(x)[1] %in% mixed_classes) & !(class(x)[1] %in% "crr2")) {
    tidy_bit <- broom::tidy(
      x,
      exponentiate = exponentiate,
      conf.level = conf.level, conf.int = T
...
parse_fit_crr2 <- function(fit, tidy, label, show_yesno) {
  # extracting model frame
  if (class(fit)[1] %in% "crr2") { # can add other classes later. Need exact subclass.
    model_frame <- attributes(fit)$model.frame
  } else model_frame <- stats::model.frame(fit)
