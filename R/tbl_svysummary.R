  ...
  if (is.null(by)) {
    ...
    if (percent == "column") {
      svy_p <- survey::svyby(c_form(right = variable), c_form(right = by), data, survey::svymean, na.rm = TRUE) %>%
        ...
        mutate(
          stat = if_else(str_sub(.data$name, 1, 3) == "se.", "p_se", "p"), # note: will fail if variable name start with "se."
      ...
      svy_p <- survey::svyby(c_form(right = by), c_form(right = variable), data, survey::svymean, na.rm = TRUE) %>%
        ...
        mutate(
          stat = if_else(str_sub(.data$name, 1, 3) == "se.", "p_se", "p"),  # note: will fail if by name start with "se."
