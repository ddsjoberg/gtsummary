...
#'
#' @section Note:
#' The N reported in the `tbl_regression_crr2()` output is the number of observations
...
#'   tbl_regression_crr2(exponentiate = TRUE)
#'
#' tbl_regression_ex2 <-
...
#'
tbl_regression_crr2 <- function(x, label = NULL, exponentiate = FALSE,
  ...
  if (class(x)[1] %in% "crr2") { # can add other classes later. Need exact subclass.
    label = names(attributes(x)$model.frame)
  } else label <- tidyselect_to_list(stats::model.frame(x), label, input_type = "label")
  ...
  table_body <- parse_fit_crr2(x, tidy_model, label, show_yesno)
  ...
  # model N
  if (class(x)[1] %in% "crr2") { # can add other classes later. Need exact subclass.
    n <- attributes(x)$model.frame %>% nrow()
  } else n <- stats::model.frame(x) %>% nrow()
