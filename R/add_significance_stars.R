...
add_significance_stars <- function(x, thresholds = c(0.001, 0.01, 0.05),
  ...
  if (pattern_cols[1] != "estimate")
    abort("The {estimate} column must be the first to appear in `pattern=`.")
  ...
  # getting character version of `table_body` ----------------------------------
  ..new_estimate_column.. <-
    x %>%
    ...
    mutate(
      ..new_estimate.. =
        ifelse(
          !is.na(.data$estimate),
          glue(.env$pattern) %>% as.character(),
          NA_character_
        )
    ) %>%
    dplyr::pull(.data$..new_estimate..)

  # replacing `estimate` column with character version -------------------------
  x <-
    x %>%
    modify_fmt_fun(estimate ~ as.character) %>%
    modify_table_body(
      ~.x %>% mutate(estimate = .env$..new_estimate_column..)
    )

  # updating hidden column status ----------------------------------------------
  x <-
    x %>%
    # hiding columns in pattern that are not the estimate
    modify_column_hide(
      columns = str_extract_all(pattern, "\\{.*?\\}") %>%
        map(str_remove_all, pattern = fixed("}")) %>%
        map(str_remove_all, pattern = fixed("{")) %>%
        unlist() %>%
        setdiff("estimate")
    ) %>%
    # hiding other columns as indicated
