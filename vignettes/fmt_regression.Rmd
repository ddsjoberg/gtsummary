...
This vignette will walk a reader through the `fmt_regression()` function, and the various functions available to modify and make additions to an existing formatted regression table. `fmt_regression()` uses `broom::tidy()` to perform the initial model formatting, and can accommodate many different model types (e.g. `lm()`, `glm()`, `coxph()`, `survreg()` and more). If your class of model is not supported by `broom::tidy()`, please request support at https://github.mskcc.org/datadojo/biostatR/issues.  In most cases, it is simple to support a new class of model.
...
The default output from `fmt_regression()` is meant to be publication ready.  Let's start by creating a regression model table from the `trial` data set included in the `clintable` package.  We will predict tumor response using age, stage, and grade using a logistic regression model.  
