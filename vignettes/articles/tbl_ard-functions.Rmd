...
The package includes functions `ard_summary()` for continuous summaries, `ard_tabulate()` for categorical summaries, and `ard_tabulate_value()` for dichotomous variables (and more).
...
- The `cardx::ard_survival_survfit()` ARD looks like the `ard_tabulate()` result.
- The `cardx::ard_stats_t_test_onesample()` ARD looks like the `ard_summary()` result.
- The `cardx::ard_categorical_ci()` ARD looks like the `ard_tabulate_value()` result.
