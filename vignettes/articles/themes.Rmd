...
To build a single table with the active theme temporarily ignored---without
clearing the theme for the rest of your session---wrap the code in
`without_gtsummary_theme()`. The expression is evaluated with the package
defaults in effect, and the active theme is restored afterward.
...
gtsummary:::df_theme_elements %>%
  ...
  dplyr::mutate(
    ...
    eval = ifelse(eval, "Yes", NA)
  ) %>%
  dplyr::relocate("eval", .after = "example") %>%
  dplyr::group_by(fn) %>%
  ...
  gt::tab_footnote(
    footnote = "Objects here are evaluated upon being read inside the package.",
    locations = gt::cells_column_labels(columns = "eval")
  ) %>%
  gt::tab_options(
