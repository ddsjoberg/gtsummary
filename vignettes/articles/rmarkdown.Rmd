...
gt_markdown_key <-
  ...
  text_transform(
    ...
    fn = function(x) {
      # the `sub()` call is adding the alt text tag to the HTML image, e.g. `<img src='smile.png' alt='smiling face emoji'>`, `gt::local_image()` returns a character string.
      dplyr::case_when(
        x == 1 ~ sub(">$", " alt=\"smiling face emoji\">", local_image(filename = path_figure[[1]], height = 20)),
        x == 2 ~ sub(">$", " alt=\"neutral face emoji\">", local_image(filename = path_figure[[2]], height = 20)),
        x == 3 ~ sub(">$", " alt=\"disappointed face emoji\">", local_image(filename = path_figure[[3]], height = 20)),
        x == 4 ~ sub(">$", " alt=\"no entry emoji\">", local_image(filename = path_figure[[4]], height = 20)),
        x == 5 ~ sub(">$", " alt=\"under construction emoji\">", local_image(filename = path_figure[[5]], height = 20))
      )
    }
  ) %>%
  tab_options(
...
tidyr::expand_grid(
  ...
  text_transform(
    ...
    fn = function(x) {
      # the `sub()` call is adding the alt text tag to the HTML image, e.g. `<img src='smile.png' alt='smiling face emoji'>`, `gt::local_image()` returns a character string.
      dplyr::case_when(
        x == 1 ~ sub(">$", " alt=\"smiling face emoji\">", local_image(filename = path_figure[[1]])),
        x == 2 ~ sub(">$", " alt=\"neutral face emoji\">", local_image(filename = path_figure[[2]])),
        x == 3 ~ sub(">$", " alt=\"disappointed face emoji\">", local_image(filename = path_figure[[3]])),
        x == 4 ~ sub(">$", " alt=\"no entry emoji\">", local_image(filename = path_figure[[4]])),
        x == 5 ~ sub(">$", " alt=\"under construction emoji\">", local_image(filename = path_figure[[5]]))
      )
    }
  ) %>%
  cols_width(
