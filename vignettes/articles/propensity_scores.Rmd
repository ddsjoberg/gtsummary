...
Randomization ensures treatment assignment will not be confounded by either measured or unmeasured baseline characteristics. In observational or non-randomized studies, treatment selection is often accompanied by subject characteristics.
...
For those who want to have deeper understanding of causal inference, please visit the causal inference R workshop prepared by Malcolm Barrett ([@malco_barrett](https://twitter.com/malco_barrett)) & Lucy D’Agostino McGowan ([@LucyStats](https://twitter.com/LucyStats)):
...
svy_nhefs <-
  ...
  mutate(
    ...
    matching_weight = 
      min(1 - ps, ps) / (qsmk * ps + (1 - qsmk) * (1 - ps))
  ) %>%
...
coxMatchedfrail <- 
  coxph(
    Surv(os.yrs, death) ~ qsmk + frailty(subclass),
    data = df_post_match
  ) %>% 
