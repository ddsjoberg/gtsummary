...
- Causal inference R workshop prepared by ([Malcolm Barrett](https://twitter.com/malco_barrett)) and  ([Lucy D’Agostino McGowan](https://twitter.com/LucyStats)). Workshop materials can be found [here](https://github.com/malcolmbarrett/causal_inference_r_workshop).
...
We've created a survey design object to handle the weights, and now we can simply use the `survey::svyglm()` and `survey::svycoxph()` functions to estimate the regression models accounting for the weighting scheme.
