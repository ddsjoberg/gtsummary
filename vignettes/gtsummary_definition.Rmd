...
Every {gtsummary} obejct has a few charactistics common among all obejcts.
Here, we review those characteristcs, and provide insturctions on how to construct a {gtsummary} object.

```{r setup, message=FALSE}
library(gtsummary); library(purrr); library(dplyr); library(tibble)
...
The `.$gt_calls` object is a list of {gt} calls saved as strings (this may be updated later to be expressions).
Every {gt} is refered to with the double colon, `::`.
The calls are executed in the order they appear in the list, and awlays begin with the `gt::gt()` call.
...
The `.$kable_calls` object is a list of data frame maninuplation calls saved as strings (this may be updated later to be expressions).
...
The `"variable"` is often used in the `inline_text()` family of functions to select the rows to print in the body of an Rmarkdown document.
...
There are a few internal {gtsummary} functions to assist in constructing and modifying a `.$table_header` dtaa frame.
...
If a column does not exist, it is populated with approriate default values.
...
Lastly, any time the `.$table_header` object is modfied, it is critical the author also runs `update_calls_from_table_header()`.
...
Within the print function the {gtsummary} obejct is converted to either a gt object or a knitr::kable object depnding on the chosen print engine.
...
The `as_gt()` and `as_kable()` function execute the calls saved in `.$gt_calls` and `.$kable_calls`, respecitvely, converting the object from gtsummary to the specifed type.
