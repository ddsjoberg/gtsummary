...
Install the {gt} package with the following command.
...
If the {gt} package is not installed, tables will be printed with the `knitr::kable()` function.
Tables printed with `knitr::kable()` are less full featured than those printed with {gt}.
For example, they do not support spanning headers, footnotes, indentation, and more.
...
- `html_document`
  - HTML output is fully supported by {gtsummary} and is the recommended output type.
- `pdf_document`
