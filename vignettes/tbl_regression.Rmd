...
It is reasonable that you'll need to modify the text.  To do this, use the `pattern` argument.  The `pattern` argument syntax follows the `glue::glue()` format with referenced R objects being inserted between curly brackets.  You have access to following fields within the `pattern` argument.  The default is `pattern = "{coef} ({conf.level*100}% CI {ll}, {ul}; {pvalue})"`
    ...
    {coef}       primary estimate (e.g. model coefficient, odds ratio)
