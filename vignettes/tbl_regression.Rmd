...
When you print output from the `tbl_regression()` function into the R console or into an R markdown document, there are default printing functions that are called in the background, e.g. `print.tbl_regression()`.
...
The `tbl_uvregression()` produces a table of univariate regression results.
