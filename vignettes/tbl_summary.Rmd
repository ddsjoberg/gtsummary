...
The {gtsummary} package accepts all select helpers available throughout the tidyverse (e.g. anything you can use with the `dplyr::select()` function can be used with {gtsummary}).
This means you have flexibility in the way you select variables for summaries, how they are displayed in the table, and other customization options.
For example, if you want to show age and the marker levels to one decimal place, you can pass `digits = c(age, marker) ~ 1` to `tbl_summary()`.
The selecting input is flexible, and you may also pass quoted column names: `digits = c("age", "marker") ~ 1`.
{gtsummary} also utilizes [{tidyselect} helpers](https://tidyselect.r-lib.org/reference/select_helpers.html), such as `starts_with()`, `contains()`, and `everything()`.

While the selectors available in the tidyverse are wonderful, there are some additional selectors specific to {gtsummary} that have been included in the package.

1. **Summary type** There are three types of summary types in {gtsummary}, and you may use the type to select columns.  This is useful, for example, when you wish to report the mean and standard deviation for all continuous variables: `statistic = all_continuous() ~ "{mean} ({sd})"`.

    ```{r, eval=FALSE}
    all_continuous()      all_categorical()      all_dichotomous()
    ```

1. **Vector class or type** Select columns based on their class or type.
...
`vars(everything(), -stage)` is be the `dplyr` equivelent of `select(everything(), -stage)`.
