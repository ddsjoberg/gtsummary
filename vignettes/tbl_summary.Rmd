...
`gt_calls` is a named list of saved gt function calls that have yet to be run. Here's an example of the first few calls saved with `tbl_summary()`:

```{r}
tbl_summary(trial2) %>% purrr::pluck("gt_calls") %>% head(n = 3)
```

The gt functions are called in the order they appear in the list, always beginning with `gt()`.  They are only run when the object is printed with `print` or `knit_print`.  Each of these printing functions is simple, consisting of `as_gt()` only.

If the user does not want a specific gt function to run, any gt call can be omitted in the `as_gt()` function by specifying the `omit` argument.  For example, the `tbl_summary()` call creates the many gt function calls: *`r tbl_summary(trial2) %>% purrr::pluck("gt_calls") %>% names() %>% paste(collapse = ", ")`*.  Any one of these can be omitted with the following code.  In this example, the default footnote will be omitted from the output.
