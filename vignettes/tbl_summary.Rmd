...
```{r}
trial %>%
  purrr::imap_dfr(~tibble::tibble(Variable = glue::glue("`{.y}`"), Label = attr(.x, "label"))) %>%
  gt::gt() %>%
...
Note the sensible defaults with this basic usage, and can be customized.
...
To compare two or more groups, include [`add_p()`](http://www.danieldsjoberg.com/gtsummary/reference/add_p.html) with the function call, which detects variable type and uses an appropriate statistical test.
...
<img src = "img/crayon-tbl_summary-args.png" width = "100%">

### {gtsummary} functions to add information

The {gtsummary} package has built-in functions for adding to results from `tbl_summary()`.
...
<img src = "img/crayon-tbl_summary-fns.png" width = "100%">
...
1. Additional [**{gtsummary} selectors**](http://www.danieldsjoberg.com/gtsummary/reference/select_helpers.html) that are included in the package to supplement tidyselect functions.
    ...
    + **Summary type** There are three types of summary types in {gtsummary}, and you may use the type to select columns.  This is useful, for example, when you wish to report the mean and standard deviation for all continuous variables: `statistic = all_continuous() ~ "{mean} ({sd})"`.
      ...
      ```
    Dichotomous variables are, by default, included with `all_categorical()`. Use `all_categorical(dichotomous = FALSE)` to exclude dichotomous variables.

<img src = "img/crayon-selectors.png" width = "100%">
...
When you print output from the `tbl_summary()` function into the R console or into an R markdown document, there are default printing functions that are called in the background, e.g. `print.tbl_summary()`.
...
The {gt} functions are called in the order they appear, always beginning with `gt::gt()`.
...
One thing to note is that unlike `tbl_summary()`, it is not possible to pass custom functions to the `statistic` argument of `tbl_svysummary()`. You must use one of the [pre-defined summary statistic functions](http://www.danieldsjoberg.com/gtsummary/dev/reference/tbl_svysummary.html#statistic-argument) (e.g. `{mean}`, `{median}`) which leverage functions from the {survey} package to calculate the correct survey statistics.  
