...
trial2 %>%
  ...
  tbl_summary(
    by = trt, # split table by treatment variable
    # change variable labels
    label = list(marker ~ "Marker", stage ~ "Clinical T Stage"),
    # change statistics printed in table
    statistic = list(all_continuous() ~ "{mean} ({sd})",
                     all_categorical() ~ "{n} / {N} ({p}%)"),
    digits = "marker" ~ c(1, 2)
  ) %>%
  # add p-values, report t-test, round large p-values to two decimal place
  add_p(test = list(marker ~ "t.test"), pvalue_fun = ~style_pvalue(.x, digits = 2)) %>%
  add_stat_label() %>%     # add statistic labels
  bold_labels() %>%        # bold variable labels
  italicize_levels() %>%   # italicize levels
  bold_p(t = 0.2) %>%      # bold p-values under a given threshold (default is 0.05)
