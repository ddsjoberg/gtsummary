...
The `tbl_summary()` function and it's related functions have sensible defaults for rounding and formatting results.  If you, however, would like to change the defaults there are a few options.  The default options can be changed for in a single script with addition an `options()` command in the script.  The defaults can also be set on the project- or user-level R profile, `.Rprofile`.
...
| Formatting and rounding percentages                 | `options(gtsummary.tbl_summary.percent_fun = style_percent)`                              | `tbl_summary()`                                                                  |
