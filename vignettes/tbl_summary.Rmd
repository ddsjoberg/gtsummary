...
The {gtsummary} package accepts all select helpers available throughout the tidyverse (think anything you can use with the `dplyr::select()` function can be used with {gtsummary}).
This means you have flexibility in the way you select variables.
...
Because the inputs to `dplyr::select()` are flexible, you could also pass the column names quoted: `digits = c("age", "marker") ~ 1`.
The functions also accept [{tidyselect} helpers](https://tidyselect.r-lib.org/reference/select_helpers.html), such as `starts_with()`, `contains()`, and `everything()`.

While the selectors available in the tidyverse are wonderful, there are some additional selectors specific to {gtsummary} that have been added to the package that allow you to select a variable by its type of class.
For example, if all continuous variables will be summarized in `tbl_summary()` as minimum and maximum, the `all_continuous()` function can be used: `statistic = all_continuous() ~ "{min} to {max}"`.
...
1. Vector class or type selectors
...
While using compound selectors, you may need to utilize the `vars()` function.
In {gtsummary}, the `vars(...)` function passes all arguments to `dplyr::select(...)`.
For example, to select all variables in the trial data except `"stage"`, use `vars(everything(), -stage)`.
