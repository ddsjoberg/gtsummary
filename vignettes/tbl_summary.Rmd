...
There are multiple  ways to specify the `statistic =` argument. The following table shows equivalent ways to specify the mean statistic for continuous variables:

```{r, echo = FALSE}
tibble::tribble(
  ~`select_helper`, ~`varname`, ~`named_list`,
  '`all_continuous() ~ "{mean}"`<br>`list(all_continuous() ~ "{mean}")`', '`c("age", "marker") ~ "{mean}"`<br>`c(age, marker) ~ "{mean}"`<br>`list(c(age, marker) ~ "{mean}")`', '`list(age = "{mean}", marker = "{mean}")`'
) %>%
  gt::gt() %>%
...
For details on creating a theme and setting personal defaults, review the [themes vignette](http://www.danieldsjoberg.com/gtsummary/dev/articles/themes.html).
...
You must use one of the [pre-defined summary statistic functions](http://www.danieldsjoberg.com/gtsummary/dev/reference/tbl_svysummary.html#statistic-argument) (e.g. `{mean}`, `{median}`) which leverage functions from the {survey} package to calculate weighted statistics.  
