...
The {gtsummary} package also supports survey data (objects created with the {survey} package) via the `tbl_svysummary()` function.
...
To begin, we'll install the {survey} package and load an example data set.
The `apiclus1` data set with a complex survey design.
...
`tbl_svysummary()` could also be used to handle weighted data. See the following datasets:
...
Each row represent zero or several individuals. If you apply `tbl_summary()` on this dataset, results will be incorrect because each row will count for one observation.

```{r}
tbl_summary(d, include = -n)
```

You can use the `survey` package to define a simple weighted dataset (with no strata and no cluster) and then apply `tbl_svysummary')`.
...
Note that, like `tbl_summary`, `tbl_svysummary()` can accept a `by` variable. `add_p()` and `add_overall()` works also with tables produced by `tbl_svysummary()`.
