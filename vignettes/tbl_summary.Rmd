...
The default output from `tbl_summary()` is meant to be publication ready.  Let's start by creating a table of summary statistics from the `trial` data set.  The `tbl_summary()` can take, at minumum, a data set as the only input, and returns descriptive statistics for each column in the data frame.
...
```{r,eval=FALSE}
options(gtsummary.tbl_summary.percent_fun = sigfig(x, digits = 3))
```
