...
test_that("combine_terms and Hmisc", {
  testthat::skip_if_not_installed("Hmisc")

  # adding splines
  mod2 <- lm(age ~ Hmisc::rcspline.eval(marker, inclx = TRUE) + stage,
             trial[c("age", "marker", "stage")] %>% na.omit())

  expect_error(
    tbl2 <- tbl_regression(mod2, label = stage ~ "Stage") %>%
      combine_terms(formula_update = . ~ . -Hmisc::rcspline.eval(marker, inclx = TRUE),
                    label = "Marker (non-linear terms)"),
    NA
  )
