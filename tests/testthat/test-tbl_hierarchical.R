...
# tbl_hierarchical(by) --------------------------------------------------------------
test_that("tbl_hierarchical(by)", {
  expect_snapshot(tbl_hierarchical(data = trial, by = trt) |> as.data.frame())
  expect_snapshot(tbl_hierarchical(data = mtcars, by = am) |> as.data.frame())
  expect_snapshot(tbl_hierarchical(data = iris, by = Species) |> as.data.frame())

  # ensure the columns appear in the correct order with 10+ by levels
  expect_equal(
    tbl_hierarchical(data.frame(x = 1, y = LETTERS[1:10]), by = y, type = x ~ "continuous") |>
  ...
  expect_equal(
    tbl_hierarchical(data.frame(x = 1, y = LETTERS[1:10]), by = y, type = x ~ "continuous2") |>
      getElement("table_body") |>
      select(all_stat_cols()) |>
      names(),
    paste0("stat_", 1:10)
  ...
  expect_equal(
    tbl_hierarchical(data.frame(x = 1, y = LETTERS[1:10]), by = y, type = x ~ "categorical") |>
      getElement("table_body") |>
      select(all_stat_cols()) |>
      names(),
    paste0("stat_", 1:10)
  ...
  expect_equal(
    tbl_hierarchical(data.frame(x = 1, y = LETTERS[1:10]), by = y, type = x ~ "dichotomous", value = x ~ 1) |>
      getElement("table_body") |>
      select(all_stat_cols()) |>
      names(),
    paste0("stat_", 1:10)
...
test_that("tbl_hierarchical(by) errors properly", {
  # errors thrown when bad data argument passed
  expect_snapshot(error = TRUE, tbl_hierarchical(mtcars, by = c("mpg", "am")))
})


# tbl_hierarchical(label) -----------------------------------------------------------
test_that("tbl_hierarchical(label)", {
  expect_error(
...
test_that("tbl_hierarchical(label) errors properly", {
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial["age"], label = list(age = letters))
  )

  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial["age"], label = letters)
  )
})

# tbl_hierarchical(statistic) -------------------------------------------------------
test_that("tbl_hierarchical(statistic)", {
  # categorical summary
  expect_equal(
    trial |>
      tbl_hierarchical(
...
test_that("tbl_hierarchical(statistic) errors properly", {
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(
      trial,
...
test_that("tbl_hierarchical(statistic,type) errors", {
...
# tbl_hierarchical(digit) -----------------------------------------------------------
test_that("tbl_hierarchical(digit)", {
  expect_error(
    tbl <- tbl_hierarchical(
      trial,
...
test_that("tbl_hierarchical(digit) errors properly", {
  expect_error(
    tbl_hierarchical(
      trial,
      include = age,
...
# tbl_hierarchical(type) ------------------------------------------------------------
test_that("tbl_hierarchical(type)", {
  expect_snapshot(
    tbl_hierarchical(
      trial,
...
test_that("tbl_hierarchical(type) proper errors/messages", {
...
# tbl_hierarchical(value) -----------------------------------------------------------
test_that("tbl_hierarchical(value)", {
  # ensure grade is coerced to dichotomous and response defaults to dichotomous
  expect_error(
    tbl <- tbl_hierarchical(trial, value = "grade" ~ "III", include = c(grade, response)),
    NA
  )
  expect_snapshot(as.data.frame(tbl))
...
test_that("tbl_hierarchical(value) errors properly", {
  # passing a value that does not exist
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial, value = "grade" ~ "IV", include = c(grade, response))
  )
})

# tbl_hierarchical(missing) ---------------------------------------------------------
test_that("tbl_hierarchical(missing)", {
...
# tbl_hierarchical(missing_text) ----------------------------------------------------
test_that("tbl_hierarchical(missing_text)", {
  expect_snapshot(
    tbl_hierarchical(
      trial,
      include = response,
...
# tbl_hierarchical(missing_stat) ----------------------------------------------------
test_that("tbl_hierarchical(missing_stat)", {
  ...
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial, include = response, missing_stat = letters)
  )
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial, include = response, missing_stat = 10L)
  )
})

# tbl_hierarchical(sort) ------------------------------------------------------------
test_that("tbl_hierarchical(sort)", {
  expect_equal(
    tbl_hierarchical(mtcars, sort = all_categorical() ~ "frequency", include = cyl) |>
      getElement("table_body") |>
...
test_that("tbl_hierarchical(sort) errors properly", {
  # proper errors are returned
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(mtcars, sort = list(all_categorical() ~ c("frequency", "two")))
  )
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(mtcars, sort = list(all_categorical() ~ "freq5555uency"))
  )
})

# tbl_hierarchical(percent) ---------------------------------------------------------
test_that("tbl_hierarchical(percent)", {
  expect_snapshot(
    tbl_hierarchical(trial, by = trt, include = grade, percent = "column", statistic = ~"{p}%") |>
      as.data.frame(col_labels = FALSE)
  )

  expect_snapshot(
    tbl_hierarchical(trial, by = trt, include = grade, percent = "row", statistic = ~"{p}%") |>
      as.data.frame(col_labels = FALSE)
  )

  expect_snapshot(
    tbl_hierarchical(trial, by = trt, include = grade, percent = "cell", statistic = ~"{p}%") |>
      as.data.frame(col_labels = FALSE)
  )
  ...
  expect_snapshot(
    error = TRUE,
    tbl_hierarchical(trial, by = trt, include = grade, percent = letters, statistic = ~"{p}%")
  )
})

test_that("tbl_hierarchical() with hms times", {
  ...
  expect_silent(
    tbl <- tbl_hierarchical(trial2, by = trt, include = time_hms)
  )
  expect_equal(
...
test_that("tbl_hierarchical() edge case of warning condition printing", {
  expect_snapshot(
    dplyr::tibble(
