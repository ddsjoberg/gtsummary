...
test_that("theme element add_ci.tbl_svysummary-arg:method", {
  ...
  expect_silent(
    tbl <-
      svy_trial2 |>
      tbl_svysummary(
        include = c(age, age_svymean, grade, grade_svyprop.logit),
        missing = "no",
        statistic = list(all_continuous() ~ "{mean}", all_categorical() ~ "{p}%")
      ) |>
      add_ci(
  ...
  expect_equal(
    tbl$table_body |>
      ...
      dplyr::pull(ci_stat_0),
    tbl$table_body |>
      dplyr::filter(variable == "grade_svyprop.logit") |>
      dplyr::pull(ci_stat_0)
  )
  ...
  expect_silent(
    list(
      "add_ci.tbl_svysummary-arg:method" = list(all_continuous() ~ "svymedian.mean", all_categorical() ~ "svyprop.beta")
    ) |>
      set_gtsummary_theme()
  )
